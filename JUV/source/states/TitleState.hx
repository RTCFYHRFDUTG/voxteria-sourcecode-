...
class TitleState extends MusicBeatState
{
	...
	var bg:FlxSprite;                          // background member
	var logoBl:FlxSprite;                      // logo member
	var baseLogoPos:FlxPoint = FlxPoint.get(); // anchor positions
	var baseBGPos:FlxPoint   = FlxPoint.get();

	var parallaxLogo:Float = 14;    // logo max shift at screen edge
	var parallaxBG:Float  = 7;      // bg max shift at screen edge
	var parallaxSmooth:Float = 10;  // smoothing factor (higher = snappier)
	...
	override public function create():Void
	{
		...
		{
    		startIntro();   // set up logo/bg etc
    		skipIntro();    // instantly remove the old credit sequence
		}
		#end
	}
	...
	function startIntro()
	{
		...
		bg = new FlxSprite(0, 0).loadGraphic(Paths.image("titleBG"));
		...
		bg.screenCenter();
		add(bg);
		baseBGPos.set(bg.x, bg.y);

		logoBl = new FlxSprite().loadGraphic(Paths.image('JUVLogo'));
		...
		logoBl.y = -logoBl.height - 20;
		add(logoBl);

		transitioning = true;
		FlxTween.tween(logoBl, {
			y: (FlxG.height - logoBl.height) / 2
		}, 2, { ease: FlxEase.quartOut, onComplete: function(_){
			baseLogoPos.set(logoBl.x, logoBl.y);
			transitioning = false;
		}});
		...
		if(Paths.fileExists('images/gfDanceTitle.json', TEXT))
		{
			...
			if(titleRaw != null && titleRaw.length > 0)
			{
				try
				{
					...
					{
						var bgJSON:FlxSprite = new FlxSprite().loadGraphic(Paths.image(titleJSON.backgroundSprite));
						bgJSON.antialiasing = ClientPrefs.data.antialiasing;
						add(bgJSON);
					}
				}
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && logoBl != null && bg != null)
		{
			var cx:Float = FlxG.width * 0.5;
			var cy:Float = FlxG.height * 0.5;
			...
			var targetLogoX:Float = baseLogoPos.x + nx * parallaxLogo;
			var targetLogoY:Float = baseLogoPos.y + ny * parallaxLogo;
			var targetBGX:Float   = baseBGPos.x   + nx * parallaxBG;
			var targetBGY:Float   = baseBGPos.y   + ny * parallaxBG;

			var a:Float = Math.min(1, parallaxSmooth * elapsed);
			logoBl.x = FlxMath.lerp(logoBl.x, targetLogoX, a);
			logoBl.y = FlxMath.lerp(logoBl.y, targetLogoY, a);
			bg.x     = FlxMath.lerp(bg.x,     targetBGX,   a);
			bg.y     = FlxMath.lerp(bg.y,     targetBGY,   a);
		}
